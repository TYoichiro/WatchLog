...
function makeRequest(params: Record<string, string> = {}) {
  const url = new URL("http://localhost/api/room/eventandsupport");
...
describe("GET /api/room/eventandsupport", () => {
  ...
  it("イベントとサポートデータを返す", async () => {
    const data = { event: { id: "e1" }, support: [] };
    mocks.getRoomEventAndSupport.mockResolvedValue(data);
  ...
  it("上流 API が失敗した場合は 502 を返す", async () => {
    mocks.getRoomEventAndSupport.mockRejectedValue(new Error("upstream error"));
