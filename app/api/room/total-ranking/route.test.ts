...
function makeRequest(params: Record<string, string> = {}) {
  const url = new URL("http://localhost/api/room/total-ranking");
...
describe("GET /api/room/total-ranking", () => {
  ...
  it("フィルタリング済みのトータルランキングを返す", async () => {
    const rawRanking = [{ rank: 1, userId: "u1" }];
    const filtered = [{ rank: 1, userId: "u1" }];
    mocks.getRoomTotalRanking.mockResolvedValue(rawRanking);
  ...
  it("上流 API が失敗した場合は 502 を返す", async () => {
    mocks.getRoomTotalRanking.mockRejectedValue(new Error("upstream error"));
